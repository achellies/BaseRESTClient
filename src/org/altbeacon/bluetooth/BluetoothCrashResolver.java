...
import com.limemobile.app.sdk.logging.LogManager;

/**
 * 
 * This class provides relief for Android Bug 67272. This bug in the Bluedroid
 * stack causes crashes in Android's BluetoothService when scanning for BLE
 * devices encounters a large number of unique devices. It is rare for most
 * users but can be problematic for those with apps scanning for Bluetooth LE
 * devices in the background (e.g. beacon-enabled apps), especially when these
 * users are around Bluetooth LE devices that randomize their mac address like
 * Gimbal beacons.
 * 
 * This class can both recover from crashes and prevent crashes from happening
 * in the first place
 * 
...
public class BluetoothCrashResolver {
    ...
    /**
     * This is not the same file that bluedroid uses. This is just to maintain
    ...
    /**
     * // It is very likely a crash if Bluetooth turns off and comes // back on
     * in an extremely short interval. Testing on a Nexus 4 shows // that when
     * the BluetoothService crashes, the time between the STATE_OFF // and the
     * STATE_TURNING_ON ranges from 0ms-684ms // Out of 3614 samples: // 99.4%
     * (3593) < 600 ms // 84.7% (3060) < 500 ms // So we will assume any power
     * off sequence of < 600ms to be a crash // // While it is possible to
     * manually turn bluetooth off then back on in // about 600ms, but it is
     * pretty hard to do. //
     */
    private static final long SUSPICIOUSLY_SHORT_BLUETOOTH_OFF_INTERVAL_MILLIS = 600l;
    /**
     * The Bluedroid stack can only track only 1990 unique Bluetooth mac
     * addresses without crashing
     */
    private static final int BLUEDROID_MAX_BLUETOOTH_MAC_COUNT = 1990;
    /**
     * The discovery process will pare back the mac address list to 256, but
     * more may be found in the time we let the discovery process run, depending
     * hon how many BLE devices are around.
     */
    private static final int BLUEDROID_POST_DISCOVERY_ESTIMATED_BLUETOOTH_MAC_COUNT = 400;
    /**
     * It takes a little over 2 seconds after discovery is started before the
     * pared-down mac file is written to persistent storage. We let discovery
     * run for a few more seconds just to be sure.
     */
    ...
    /**
     * Constructor should be called only once per long-running process that does
     * Bluetooth LE scanning. Must call start() to make it do anything.
     * 
     * @param context
    ...
    /**
     * Starts looking for crashes of the Bluetooth LE system and taking
     * proactive steps to stop crashes from happening. Proactive steps require
     * calls to notifyScannedDevice(Device device) so that crashes can be
    ...
    /**
     * Stops looking for crashes. Does not need to be called in normal
     * operations, but may be useful for testing.
     */
    public void stop() {
     ...
     * 
     * @deprecated Since the default logger used by the android-beacon-library
     *             only logs warnings and above, this method is no logger used.
     *             To log debug messages use the
     *             {@link com.limemobile.app.sdk.logging.Loggers#verboseLogger()}
     * @see com.limemobile.app.sdk.logging.LogManager
     * @see com.limemobile.app.sdk.logging.Loggers
     */
    @Deprecated
    ...
    /**
     * Call this method from your BluetoothAdapter.LeScanCallback method. Doing
     * so is optional, but if you do, this class will be able to count the
     * number of disctinct bluetooth devices scanned, and prevent crashes before
     * they happen.
     * 
     * This works very well if the app containing this class is the only one
     * running bluetooth LE scans on the device, or it is constantly doing scans
     * (e.g. is in the foreground for extended periods of time.)
     * 
     * This will not work well if the application using this class is only
     * scanning periodically (e.g. when in the background to save battery) and
     * another application is also scanning on the same device, because this
     * class will only get the counts from this application.
     * 
     * Future augmentation of this class may improve this by somehow
     * centralizing the list of unique scanned devices.
     * 
     * @param device
     */
    @TargetApi(18)
    public void notifyScannedDevice(BluetoothDevice device,
        ...
        if (oldSize != newSize && newSize % 100 == 0) {
            LogManager.d(TAG, "Distinct bluetooth devices seen: %s",
        ...
        if (distinctBluetoothAddresses.size() > getCrashRiskDeviceCount()) {
            if (PREEMPTIVE_ACTION_ENABLED && !recoveryInProgress) {
                LogManager
                        .w(TAG,
                                "Large number of bluetooth devices detected: %s Proactively "
    ...
    public void crashDetected() {
        if (android.os.Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR2) {
            LogManager
                    .d(TAG,
                            "Ignoring crashes before SDK 18, because BLE is unsupported.");
            return;
        }
        ...
        if (recoveryInProgress) {
            LogManager
                    .d(TAG,
                            "Ignoring bluetooth crash because recovery is already in progress.");
        } else {
            startRecovery();
        }
        processStateChange();
    ...
    private int getCrashRiskDeviceCount() {
        // 1990 distinct devices tracked by Bluedroid will cause a crash. But we
        // don't know how many
        // devices bluedroid is tracking, we only know how many we have seen,
        // which will be smaller
        // than the number tracked by bluedroid because the number we track does
        // not include its
        // initial state. We therefore assume that there are some devices being
        // tracked by bluedroid
        // after a recovery operation or on startup
        return BLUEDROID_MAX_BLUETOOTH_MAC_COUNT
    ...
    private void startRecovery() {
        // The discovery operation will start by clearing out the bluetooth mac
        // list to only the 256
        ...
        if (!adapter.isDiscovering()) {
            ...
            if (!adapter.startDiscovery()) {
                LogManager.w(TAG,
                        "Can't start discovery.  Is bluetooth turned on?");
            }
            LogManager.d(TAG, "startDiscovery commanded.  isDiscovering()=%s",
                    adapter.isDiscovering());
            // We don't actually need to do a discovery -- we just need to kick
            // one off so the
            // mac list will be pared back to 256. Because discovery is an
            // expensive operation in
            // terms of battery, we will cancel it.
            if (TIME_TO_LET_DISCOVERY_RUN_MILLIS > 0) {
                LogManager
                        .d(TAG,
                                "We will be cancelling this discovery in %s milliseconds.",
                                TIME_TO_LET_DISCOVERY_RUN_MILLIS);
                cancelDiscovery();
            } else {
            ...
            LogManager.w(TAG,
                    "Already discovering.  Recovery attempt abandoned.");
        }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(BluetoothAdapter.ACTION_DISCOVERY_FINISHED)) {
                if (recoveryInProgress) {
                    ...
                    LogManager
                            .d(TAG, "Bluetooth discovery finished (external)");
                }
            }
            ...
            if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
                ...
                case BluetoothAdapter.STATE_ON:
                    ...
                    LogManager.d(TAG,
                            "Bluetooth was turned off for %s milliseconds",
                            lastBluetoothTurningOnTime - lastBluetoothOffTime);
    ...
    private void saveState() {
        ...
        try {
            outputStream = context.openFileOutput(
                    DISTINCT_BLUETOOTH_ADDRESSES_FILE, Context.MODE_PRIVATE);
            ...
            LogManager.w(TAG, "Can't write macs to %s",
        ...
        LogManager.d(TAG, "Wrote %s bluetooth addresses",
                distinctBluetoothAddresses.size());
    ...
    private void loadState() {
        ...
        try {
            inputStream = context
                    .openFileInput(DISTINCT_BLUETOOTH_ADDRESSES_FILE);
            ...
            LogManager.w(TAG, "Can't read macs from %s",
                    DISTINCT_BLUETOOTH_ADDRESSES_FILE);
        } catch (NumberFormatException e) {
            LogManager.w(TAG, "Can't parse file %s",
                    DISTINCT_BLUETOOTH_ADDRESSES_FILE);
        } finally {
            if (reader != null) {
                try {
        ...
        LogManager.d(TAG, "Read %s bluetooth addresses",
                distinctBluetoothAddresses.size());
    }
    ...
    private void cancelDiscovery() {
        try {
            ...
            if (!discoveryStartConfirmed) {
                LogManager
                        .w(TAG,
                                "BluetoothAdapter.ACTION_DISCOVERY_STARTED never received.  Recovery may fail.");
            }
